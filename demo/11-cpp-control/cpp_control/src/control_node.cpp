// demo/11 - C++ 控制循环(rclcpp): 高频定时器跑一个 PID, 测实际频率/抖动/耗时.
//
// 和 Python 版(../control_py.py)逻辑完全相同, 只换语言. 用 C++ 写控制循环,
// 目标频率能更接近 1000 Hz, 抖动更小, 单次计算快 1~2 个数量级.
...
private:
  void tick() {
    ...
    // ---- 假装的控制计算: PID 跟踪目标 1.0(和 Python 版完全相同) ----
    double err = 1.0 - q_;
