...
- 7 个可靠性点：校验 / 多版本 / 换版切换 / 回滚 / 事件 / 遮蔽 / 回收。
...
2. **多版本 / 换版 / 回滚**：见 `hotplug_walkthrough.md`（本会话已真实演示）。
3. **双臂可视化闭环**：web 面板改配置/小球位置 → MuJoCo 窗口实时跟随。

## 与最终目标什么关系

- 本 demo 是项目新颖性主张的**教学演示形态**(动态插件: `cordis_define`/`cordis_run`/`cordis_stop`,
  进程内临时、重启消失), 演示 7 个可靠性点与「能力热插拔」的机制直觉。
- 项目的**生产实现形态**在 `src/`(能力仓库目录 + 挂载服务 + 臂/感知作用域 + web 面板): 准入
  校验、运行时挂/卸、多版本换挂、失败回滚等同一套语义, 以挂载体系落地为常驻插件(树外包)。
- 两种形态的机制对应:

| 机制 | demo/13(演示形态) | src/(实现形态) |
|---|---|---|
| 能力形态 | 动态插件 host 半部(进程内) | 能力仓库目录 `repo/<cap>/<version>/{host.js, manifest.json}` |
| 挂载/卸载 | `cordis_run` / `cordis_stop` | web 面板 → 挂载服务 → 臂/感知上下文 `ctx.plugin`/`fiber.dispose` |
| 版本换挂/回滚 | package 版本 + run 指针 | 版本目录 + 挂载句柄(失败自动恢复旧实例) |
| 准入校验 | `mount_guard.py` 手工哈希比对 | 挂载服务内联 sha256 + kind 路由 |

- 面试时「运行时换末端执行器 + 零信任校验 + 换版回滚」以 `src/` 挂载体系 + 本 demo 走查为证据。
