...
| 目录 | 内容 |
|---|---|
| `capabilities/` | 能力仓库(`repo/`) + 挂载服务(`mount_service/`, 准入 + 臂/槽上下文管理)+ 规范 + `pack.sh` |
...
3. web 面板(人的写入口)臂 A 下拉框选「grasp 1.0.0」: 准入检查 -> 该会话臂作用域挂实例 ->
...
6. 下拉框选「不装配」: 臂层 dispose 无残留; 挂坏版本(篡改 manifest): 准入拒绝.
```
...
- 消息契约唯一自造 API 在 `bridge/`(见 `contract.md`).
