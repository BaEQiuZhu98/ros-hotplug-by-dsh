# src — 源码工程骨架(阶段 0 固化基线)

本目录是设计文档 `docs/design.zh.md` §10.1 目录结构的落点. 阶段 0 的目标:
把 demo 11~13 的产物固化为正式包, 且**不引入任何新决策**.
...
| `capabilities/` | 能力包(树外包): capability-spec v1 + `grasp` 包 v1.1.0 + `mount_guard.py` + `pack.sh`(阶段 1 已本地发布并全链路验证) | demo/13 固化 + 阶段 1 |
| `presets/robo/` | agent preset 占位(阶段 2) | - |
...
## 阶段 0 验证(一个普通 Python 脚本经 SDK 驱动 sim_bridge)
