[English](novelty.md) | 中文

# 现状分析与项目亮点

> 目的：分析现有机器人热插拔方法的局限（论文/源码/架构依据），并说清本项目的亮点与主张边界。
...
- **局限**：解决了「让 agent 调 ROS2」，但提供的是 skill 库 + 调用协议，**没有 DSH 那套「分层作用域 + Cordis dispose」的组合原语**——「工具化」上重叠，「空间可见性 + 时间精确生灭 + 版本时序」锚点上不具备。
...
> 收敛成一句话：现有方案要么管「进程/节点」，要么管「组件图」，要么管「skill 调用」，但没有一个把「此刻对谁可见（空间）」「何时生、何时被精确回收（时间）」「如何多版本共存与回滚」三者绑在同一个锚点上。
...
|---|---|---|---|
| ROS2 生命周期/可组合节点 | ROS 组件生命周期 | 机器人侧「局部热插拔」官方机制 | 只管理节点状态机，不解决 agent 可见性、同型隔离、版本回滚 |
...
- 能力实例位于 agent 之下的**臂作用域**（每会话一套：全局 → agent → 臂 → 实例）或 **agent 层的感知槽**（sensor 类）；agent 只感知 `ready` 布尔并经 `take_object` 执行，夹取/吸附策略在实例内部；同级臂作用域让同名实例并存不串台。
...
### 3.3 版本时序：版本目录 + 挂载句柄（换版 + 回滚）

- 能力仓库按版本目录存放不可变代码；挂载服务持挂载句柄：换版 = 卸载旧 + 挂新（失败自动恢复旧实例(尽力, 恢复失败显式告警) = 回滚）。
- 动态插件的 plugin（实例）/ package（不可变版本）/ run（激活尝试）三级语义是同一机制在调试形态下的体现。
- **优势**：多版本共存 + 换版 + 回滚内建，不是应用层手写。
...
| AICA | 组件图 | ✗ | 部分 | ✗ | ✗ |
| OpenRAL | skill 调用 | ✗ | ✗ | ✗ | ✓ |
| **DSH 时空组合性** | **能力工具** | **✓ 臂作用域隔离** | **✓ dispose** | **✓ 版本目录 + 句柄** | **✓** |
...
| 要件 | 内容 |
|---|---|
| 机制 | DSH 分层作用域（臂作用域/感知槽）+ 父链继承 + Cordis dispose + 能力版本目录与挂载句柄 |
| 场景 | 具身机器人能力热插拔（末端执行器/传感器） |
...
- 可复现：按 `demo/13-hotplug` README 跑通并复现热插拔验收指标（design §11.3）。
