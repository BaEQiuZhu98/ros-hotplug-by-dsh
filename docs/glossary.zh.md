...
| 作用域（scope）/ 层（layer） | 能力分层的摆放位置：全局层 → preset 常驻层 → 每 agent 层 → 臂层 | 末端实例挂臂作用域、sensor 挂感知槽，工具可见性沿父链向下继承 |
| 父链继承 | 注册视图向下继承：子作用域看得见祖先各层 | agent 看得见 preset 的能力 |
| nearest-wins（遮蔽） | 同名能力时最近者胜出（DSH 机制） | 本项目以臂作用域隔离实现同名实例不串台 |
| isolate realm（隔离域） | 每个挂载会话一份私有实例，同类服务不串台（DSH 机制） | 本项目以臂作用域隔离实现同类实例不串台 |
...
| plugin / package / run | 动态插件形态的版本时序：插件实例 / 不可变代码版本 / 一次激活尝试 | 生产形态用能力版本目录 + 挂载句柄实现换版/回滚 |
...
| 能力挂载服务 | host 常驻插件：准入检查（sha256 + kind 路由）+ 臂/槽上下文管理 + 在对应上下文上 ctx.plugin/dispose；唯一写入口 = web 面板 RPC，**不注册 agent 工具** | `src/capabilities/mount_service/` |
| 臂作用域 | 每条机械臂一个子作用域（createScope(armManagerCtx, armKey, {parent: 会话 agent 作用域})，每会话一套）：末端实例挂在这里，同名实例互不串台 | 热插拔的「空间锚点」 |
| 臂管理器 | 会话内插件：为每个会话建立臂作用域与感知槽并注册上下文（事件驱动 + 执行时懒补建兜底）；提供 arm_status/take_object | `src/presets/robo` |
...
| Client↔Host RPC | 动态插件 client 用 `host.call` 调 host 的 `harness.handle` 方法 | 本项目面板 client 经同源 fetch 调 host 的 /cap-mount 路由 |
...
| 同名隔离 / 硬件差异屏蔽 | 同型能力同名实例按臂作用域隔离、不串台 | 两个夹爪实例 |
| 无泄漏 | 臂作用域 + Cordis dispose 精确回收 | 卸载后无残留 |
| 插入即见 / 拔出即回收 / 同名不串台 / agent 无感切换 / 失败回滚 | 五项验收指标 | 评测 hotplug 维度 |
...
| 桥接契约（bridge contract） | 话题/消息 schema 文档 | `src/bridge/contract.md` |
...
| 评测三维度 | robot / agent / hotplug | `eval/` |
| 公开基线 | 1kHz、IK 求解器量级、<1mm 轨迹等可查证指标 | 设计文档 §11.2 |
| agent 感知与自适应 | agent 不控制末端，而是感知末端状态、对同一命令自适应选策略 | 设计文档 §7.4 |

## 5. agent / LLM 名词（简列）

ReAct 循环、CoT / reasoning_content、reasoning 回传规则、token 统计与优化、token-meter、压缩（compaction）、Agent 创建、Subagent 委托、Goal 目标循环、Plan 模式、多模型路由、Headless 模式、权限/审批/沙箱、Retry 策略、MCP 客户端、Client 插件/slots——详见 `demo/README.zh.md`。
